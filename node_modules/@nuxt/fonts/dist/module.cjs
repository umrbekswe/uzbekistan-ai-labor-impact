...
async function generateFontFallbacks(family, data, fallbacks) {
  ...
  const metrics = await fontaine.getMetricsForFamily(family) || fontURL && await fontaine.readMetrics(fontURL.originalURL || fontURL.url);
  if (!metrics)
    return [];
  const css = [];
  for (const fallback of fallbacks) {
    css.push(fontaine.generateFontFace(metrics, {
      ...fallback,
      metrics: await fontaine.getMetricsForFamily(fallback.font) || void 0
    }));
  }
  return css;
}
...
function parseFont(font) {
  if (font.startsWith("/") || ufo.hasProtocol(font)) {
    const extension = pathe.extname(font).slice(1);
...
function relativiseFontSources(font, relativeTo) {
  return {
    ...
    src: font.src.map((source) => {
      ...
      return {
        ...source,
        url: pathe.relative(relativeTo, source.url)
      };
    })
  };
}

const local = unifont.defineFontProvider("local", () => {
  ...
  function lookupFont(family, suffixes) {
    ...
    for (const path of paths) {
      ...
      fonts.add(base ? ufo.withLeadingSlash(pathe.relative(base, path)) : path);
    }
    return [...fonts].sort((a, b) => {
      const extA = pathe.extname(a);
      const extB = pathe.extname(b);
  ...
  nuxt.hook("nitro:init", async (nitro) => {
    for (const assetsDir of nitro.options.publicAssets) {
      const possibleFontFiles = await tinyglobby.glob(["**/*.{ttf,woff,woff2,eot,otf}"], {
        absolute: true,
        cwd: assetsDir.dir
      });
      providerContext.rootPaths.push(ufo.withTrailingSlash(assetsDir.dir));
      for (const file of possibleFontFiles) {
        registerFont(file.replace(assetsDir.dir, pathe.join(assetsDir.dir, assetsDir.baseURL || "/")));
      }
    }
    providerContext.rootPaths = providerContext.rootPaths.sort((a, b) => b.length - a.length);
  });
  nuxt.hook("builder:watch", (event, relativePath) => {
    relativePath = pathe.relative(nuxt.options.srcDir, pathe.resolve(nuxt.options.srcDir, relativePath));
...
const WEIGHT_RE = magicRegexp.createRegExp(magicRegexp.anyOf(.../* @__PURE__ */ new Set([...weights, ...weights.map((w) => w.replace("-", ""))])).groupedAs("weight").after(magicRegexp.not.digit).before(magicRegexp.not.digit.or(magicRegexp.wordBoundary)), ["i"]);
const styles = ["italic", "oblique"];
const STYLE_RE = magicRegexp.createRegExp(magicRegexp.anyOf(...styles).groupedAs("style").before(magicRegexp.not.wordChar.or(magicRegexp.wordBoundary)), ["i"]);
const subsets = [
  "cyrillic-ext",
  "cyrillic",
  "greek-ext",
...
const SUBSET_RE = magicRegexp.createRegExp(magicRegexp.anyOf(...subsets).groupedAs("subset").before(magicRegexp.not.wordChar.or(magicRegexp.wordBoundary)), ["i"]);
...
const storage = unstorage.createStorage({
  driver: fsDriver__default({ base: cacheBase })
});
...
const FontFamilyInjectionPlugin = (options) => unplugin.createUnplugin(() => {
  let postcssOptions;
  async function transformCSS(code, id, opts = {}) {
    ...
    async function addFontFaceDeclaration(fontFamily, fallbackOptions) {
      ...
      for (const font of result.fonts) {
        ...
        const declarations = [generateFontFace(fontFamily, opts.relative ? relativiseFontSources(font, ufo.withLeadingSlash(pathe.dirname(id))) : font), ...fallbackDeclarations];
        for (let declaration of declarations) {
          if (!injectedDeclarations.has(declaration)) {
            injectedDeclarations.add(declaration);
            if (!options.dev) {
              declaration = await esbuild.transform(declaration, {
                loader: "css",
                charset: "utf8",
                minify: true,
                ...postcssOptions
    ...
    function processNode(node, parentOffset = 0) {
      ...
      cssTree.walk(node, {
        visit: "Declaration",
        enter(node2) {
          if (node2.property !== "font-family" && node2.property !== "font" && (!options.processCSSVariables || !node2.property.startsWith("--")) || this.atrule?.name === "font-face") {
            return;
          }
      ...
      cssTree.walk(node, {
        visit: "Raw",
        enter(node2) {
          const nestedRaw = cssTree.parse(node2.value, { positions: true });
...
const logger = kit.useLogger("@nuxt/fonts");
...
async function setupPublicAssetStrategy(options = {}) {
    ...
    for (const face of Array.isArray(faces) ? faces : [faces]) {
      data.push({
        ...
        src: (Array.isArray(face.src) ? face.src : [face.src]).map((src) => {
          ...
          if ("url" in source && ufo.hasProtocol(source.url, { acceptRelative: true })) {
            source.url = source.url.replace(/^\/\//, "https://");
            const file = [
              // TODO: investigate why negative ignore pattern below is being ignored
              utils.filename(source.url.replace(/\?.*/, "")).replace(/^-+/, ""),
              ohash.hash(source) + (pathe.extname(source.url) || formatToExtension(source.format) || "")
            ].filter(Boolean).join("-");
            renderedFontURLs.set(file, source.url);
            source.originalURL = source.url;
            source.url = nuxt.options.dev ? ufo.joinRelativeURL(nuxt.options.app.baseURL, assetsBaseURL, file) : ufo.joinURL(assetsBaseURL, file);
            if (!nuxt.options.dev) {
              node_fs.writeFileSync(pathe.join(cacheDir, file), "");
            }
          }
          return source;
        })
  ...
  async function devEventHandler(event) {
    ...
    if (!res) {
      res = await proxy.fetch(url).then((r) => r.arrayBuffer()).then((r) => Buffer.from(r));
      await storage.setItemRaw(key, res);
    }
    return res;
  }
  kit.addDevServerHandler({
    route: ufo.joinURL(nuxt.options.runtimeConfig.app.baseURL || nuxt.options.app.baseURL, assetsBaseURL),
    handler: h3.eventHandler(devEventHandler)
  });
  kit.addVitePlugin({
  ...
  if (nuxt.options.dev) {
    ...
    nuxt.options.routeRules[ufo.joinURL(assetsBaseURL, "**")] = {
      cache: {
  ...
  const cacheDir = pathe.join(nuxt.options.buildDir, "cache", "fonts");
  if (!nuxt.options.dev) {
    await fsp__default.mkdir(cacheDir, { recursive: true });
  }
  nuxt.options.nitro = defu.defu(nuxt.options.nitro, {
    publicAssets: [{
      dir: cacheDir,
      maxAge: ONE_YEAR_IN_SECONDS,
      baseURL: assetsBaseURL
    }],
    ignore: [`!${pathe.join(cacheDir, "**/*")}`],
    prerender: {
  ...
  nuxt.hook("nitro:init", (nitro) => {
    ...
    nitro.hooks.hook("rollup:before", async () => {
      ...
      await fsp__default.rm(cacheDir, { recursive: true, force: true });
      await fsp__default.mkdir(cacheDir, { recursive: true });
      let banner = false;
      for (const [filename2, url] of renderedFontURLs) {
        ...
        if (!res) {
          ...
          logger.log(chalk__default.gray("  \u251C\u2500 " + url));
          res = await proxy.fetch(url).then((r) => r.arrayBuffer()).then((r) => Buffer.from(r));
          await storage.setItemRaw(key, res);
        }
        await fsp__default.writeFile(pathe.join(cacheDir, filename2), res);
      }
      if (banner) {
...
function setupDevtoolsConnection(enabled) {
  ...
  let rpc;
  const fonts = [];
  devtoolsKit.onDevToolsInitialized(() => {
    rpc = devtoolsKit.extendServerRpc(DEVTOOLS_RPC_NAMESPACE, {
      getFonts: () => fonts,
      generateFontFace
    });
...
const module$1 = kit.defineNuxtModule({
  ...
  defaults: {
    ...
    providers: {
      local,
      adobe: unifont.providers.adobe,
      google: unifont.providers.google,
      googleicons: unifont.providers.googleicons,
      bunny: unifont.providers.bunny,
      fontshare: unifont.providers.fontshare,
      fontsource: unifont.providers.fontsource
    }
  },
  async setup(options, nuxt) {
    ...
    async function resolveFontFaceWithOverride(fontFamily, override, fallbackOptions) {
      ...
      if (override?.provider) {
        if (override.provider in providers2) {
          const result2 = await unifont$1.resolveFont(fontFamily, defaults, [override.provider]);
      ...
      const result = await unifont$1.resolveFont(fontFamily, defaults, [...prioritisedProviders]);
      if (result) {
    ...
    nuxt.hook("vite:extend", (ctx) => {
      viteEntry = pathe.relative(ctx.config.root || nuxt.options.srcDir, ctx.entry);
    });
    nuxt.hook("build:manifest", (manifest) => {
      ...
      function addPreloadLinks(chunk, urls) {
        ...
        for (const url of urls) {
          ...
          if (!manifest[url]) {
            manifest[url] = {
              file: pathe.relative(nuxt.options.app.buildAssetsDir, url),
              resourceType: "font",
              preload: true
            };
          }
      ...
      for (const chunk of Object.values(manifest)) {
        ...
        for (const css of chunk.css) {
          const assetName = ufo.withoutLeadingSlash(pathe.join(nuxt.options.app.buildAssetsDir, css));
      ...
      for (const [id, urls] of fontMap) {
        const chunk = manifest[pathe.relative(nuxt.options.srcDir, id)];
        if (!chunk)
          continue;
    ...
    kit.addBuildPlugin(FontFamilyInjectionPlugin({
...
async function resolveProviders(_providers = {}) {
  const nuxt = kit.useNuxt();
  const jiti$1 = jiti.createJiti(nuxt.options.rootDir, { alias: nuxt.options.alias });
  ...
  for (const key in providers2) {
    ...
    if (typeof value === "string") {
      providers2[key] = await jiti$1.import(value, { default: true });
    }
  }
  return providers2;
}
